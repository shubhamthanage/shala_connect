...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-navy">
      ...
      <ScrollReveal>
        <div className="bg-gradient-to-r from-saffron via-amber-500 to-gold py-8 sm:py-6 px-4 sm:px-6 md:px-12 lg:px-16 flex justify-center relative overflow-hidden">
          <div className="absolute inset-0 bg-[radial-gradient(ellipse_80%_50%_at_50%_50%,rgba(255,255,255,0.15),transparent)] pointer-events-none" />
          <div className="max-w-[1400px] w-full flex flex-wrap justify-center gap-6 sm:gap-8 md:gap-0 relative z-10">
            {[
              { n: 50000, l: "शाळा वापरतात", format: (v: number) => `${Math.round(v / 1000).toLocaleString("mr-IN")},०००+` },
              { n: 2500000, l: "विद्यार्थी", format: (v: number) => `${Math.round(v / 100000).toLocaleString("mr-IN")} लाख+` },
              { n: 99.9, l: "Uptime गॅरंटी", format: (v: number) => `${v.toFixed(1)}%` },
              { n: 36, l: "जिल्हे सक्रिय", format: (v: number) => `${Math.round(v).toLocaleString("mr-IN")}` },
              { n: 4.9, l: "App Store Rating", format: (v: number) => `${v.toFixed(1)} ★` },
            ].map((item, i) => (
              <div key={i} className="flex-1 min-w-[100px] xs:min-w-[120px] text-center relative last:after:hidden md:after:content-[''] md:after:absolute md:after:right-0 md:after:top-[15%] md:after:bottom-[15%] md:after:w-px md:after:bg-white/30">
                <div className="font-extrabold text-white text-xl sm:text-2xl md:text-3xl font-heading drop-shadow-sm">
                  <CountUp end={item.n} duration={2000} format={item.format} />
                </div>
                <div className="text-xs text-white/80 mt-1 font-body">{item.l}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Trusted by — logos */}
      <ScrollReveal>
        <div className="py-12 px-6 md:px-[60px] bg-cream border-b border-border-school">
          <p className="text-center text-[11px] font-extrabold tracking-[2px] uppercase text-text-300 mb-6 font-sora">
            महाराष्ट्रातील शाळा आमच्यावर विश्वास ठेवतात
          </p>
          <div className="max-w-[1000px] mx-auto flex flex-wrap items-center justify-center gap-10 md:gap-16 opacity-70">
            {["MSBSHSE", "पुणे जिल्हा", "नाशिक शिक्षण", "औरंगाबाद ZP", "मुंबई BMC"].map((name, i) => (
              <div key={i} className="px-6 py-2 rounded-xl bg-white border border-border-school shadow-sh-sm font-bold text-text-700 text-sm font-body">
                {name}
              </div>
            ))}
          </div>
      ...
      {/* Roles Section — भूमिका */}
      <ScrollReveal>
        <section id="roles" className="bg-white py-14 sm:py-20 md:py-[88px] px-4 sm:px-6 md:px-[60px] scroll-mt-24 shadow-[0_-4px_24px_rgba(5,15,30,0.06)]">
          <div className="max-w-[1400px] mx-auto">
          <span className="section-label mb-3 block">वापरकर्ते</span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 md:gap-[18px] mt-[50px]">
            {[
              { emoji: "👨‍💼", name: "मुख्याध्यापक", desc: "संपूर्ण शाळेचे नियंत्रण, अहवाल, परवानग्या", tag: "१५+ मॉड्यूल्स", hover: "saffron" },
              { emoji: "👩‍🏫", name: "शिक्षक", desc: "हजेरी, गुण, गृहपाठ, विद्यार्थी प्रगती", tag: "२०+ मॉड्यूल्स", hover: "green" },
              { emoji: "🧑‍💻", name: "कारकून", desc: "दाखले, शुल्क, दस्तऐवज, सरकारी अहवाल", tag: "१८+ मॉड्यूल्स", hover: "sky" },
              { emoji: "👦", name: "विद्यार्थी", desc: "वेळापत्रक, गृहपाठ, निकाल, ई-लायब्ररी", tag: "१२+ मॉड्यूल्स", hover: "violet" },
              { emoji: "👨‍👩‍👦", name: "पालक", desc: "हजेरी, गुण, शुल्क, शिक्षक भेट बुकिंग", tag: "१०+ मॉड्यूल्स", hover: "pink" },
            ].map((role, i) => (
              <div
                key={i}
                className="card-accent-lg p-6 md:p-7 text-center cursor-pointer"
              >
                <span className="text-4xl md:text-5xl block mb-3 drop-shadow-md">{role.emoji}</span>
                <div className="font-bold text-text-900 text-base mb-2 font-body">{role.name}</div>
                <div className="text-xs text-text-500 leading-relaxed mb-4 font-body">{role.desc}</div>
                <span className="inline-block px-3 py-1.5 rounded-full text-[10px] font-bold bg-cream text-text-500 border border-border-school font-body">
                  {role.tag}
                </span>
              </div>
            ))}
      ...
      {/* How it works */}
      <ScrollReveal>
        <section id="how-it-works" className="bg-cream py-14 sm:py-20 md:py-[88px] px-4 sm:px-6 md:px-[60px] scroll-mt-20">
        <div className="max-w-[1400px] mx-auto">
          <span className="section-label mb-3 block">कसे वापरावे</span>
          <h2 className="text-3xl md:text-[40px] font-extrabold text-text-900 mb-4 font-heading">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 relative mt-[52px]">
            ...
            {[
              ...
              <div key={i} className="text-center relative z-10">
                <div className={`w-12 h-12 rounded-full flex items-center justify-center font-extrabold text-lg mx-auto mb-4 border-[3px] border-white text-white shadow-lg`}
                  style={{
                ...
                <div className="font-bold text-text-900 text-[15px] mb-2 font-body">{step.title}</div>
                <div className="text-xs text-text-500 leading-relaxed font-body">{step.desc}</div>
              </div>
            ))}
          </div>
        </div>
